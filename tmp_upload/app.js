...
async function exportSolutionPDF() {
  if (!generatedSolution) {
    showToast('请先生成功能方案');
    return;
  }
  ...
  try {
    ...
    const evaluation = evaluatedTopics.find(topic => topic.title === chosenTopic);
    const scores = {
      hardware_score: evaluation?.hardware_score ?? '—',
      software_score: evaluation?.software_score ?? '—',
      total_score: evaluation?.total_score ?? '—',
      conclusion: evaluation?.conclusion ?? '未评估',
      reason: evaluation?.reason ?? '',
    };

    // 收集元器件数据
    const components = {
      sensors: generatedSolution.sensors || {},
      displays: generatedSolution.displays || {},
      actuators: generatedSolution.actuators || {},
    };

    // 发送到后端生成 PDF
    const response = await fetch('/api/solution/pdf', {
      method: 'POST',
      headers: { 'Content-Type': 'application/json' },
      body: JSON.stringify({
        topic: generatedSolution.topic,
        scores,
        components,
        design_logic: generatedSolution.design_logic || {},
      }),
    });

    if (!response.ok) {
      const error = await response.json().catch(() => ({ error: 'PDF 生成失败' }));
    ...
    a.download = `${safeTopic}_功能方案.pdf`;
    document.body.appendChild(a);
    a.click();
    document.body.removeChild(a);
    URL.revokeObjectURL(url);
    showToast('功能方案 PDF 已下载');
  } catch (error) {
    showToast(error.message);
  } finally {
    if (button) {
      button.disabled = false;
      button.textContent = '⇩ 导出为 PDF';
    }
  }
}
