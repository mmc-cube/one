...
def escape_pdf(text: str) -> str:
    """Replace characters that fpdf2 can't render (emoji / special symbols)."""
...
def generate_solution_pdf(
    ...
    design_logic: dict[str, object],
) -> io.BytesIO | None:
    """Build a clean A4 PDF with the solution content, return in-memory buffer."""
    try:
    ...
    pdf.cell(0, 5, "白砾电子 · 功能方案", align="C")
    pdf.ln(8)

    # ── 标题 ──
    pdf.set_text_color(0, 0, 0)
    pdf.set_font("cjk", "", 14)
    pdf.multi_cell(0, 8, escape_pdf(topic))
    pdf.ln(4)

    # ── 评分摘要 ──
    hw = scores.get("hardware_score", "—")
    sw = scores.get("software_score", "—")
    total = scores.get("total_score", "—")
    conclusion = str(scores.get("conclusion", "—"))
    reason = str(scores.get("reason", ""))

    pdf.set_font("cjk", "", 10)
    pdf.set_fill_color(245, 247, 250)
    pdf.cell(0, 7, "  选题评估", fill=True)
    pdf.ln(9)
    pdf.set_font("cjk", "", 9)
    labels = (
        f"硬件可完成度：{hw}　　软件可完成度：{sw}　　综合评分：{total}　　结论：{conclusion}"
    )
    pdf.cell(0, 6, labels)
    pdf.ln(7)
    if reason:
        ...
        pdf.multi_cell(0, 5, f"评估说明：{escape_pdf(reason)}")
        pdf.ln(2)
    pdf.set_text_color(0, 0, 0)
    pdf.ln(3)
    ...
    sections: list[tuple[str, str, object]] = [
        ("传感器方案", "sensors", components.get("sensors")),
        ("显示屏方案", "displays", components.get("displays")),
        ("执行驱动器方案", "actuators", components.get("actuators")),
    ]

    for label, _key, section in sections:
        if not isinstance(section, dict):
            continue
        items = section.get("components")
        if not isinstance(items, list) or not items:
            continue

        pdf.set_font("cjk", "", 10)
        pdf.set_fill_color(245, 247, 250)
        pdf.cell(0, 7, f"  {label}（共 {len(items)} 个）", fill=True)
        pdf.ln(10)
        for item in items:
            name = str(item.get("name", ""))
            desc = str(item.get("description", ""))
            pdf.set_font("cjk", "", 10)
            pdf.cell(0, 6, f"● {escape_pdf(name)}")
            pdf.ln(7)
            if desc:
                pdf.set_font("cjk", "", 9)
                pdf.set_text_color(60, 60, 60)
                pdf.set_x(14)
                pdf.multi_cell(0, 5, escape_pdf(desc))
                pdf.ln(2)
            pdf.set_text_color(0, 0, 0)
        pdf.ln(3)

    # ── 功能设计 ──
    if isinstance(design_logic, dict):
        pdf.set_font("cjk", "", 10)
        pdf.set_fill_color(245, 247, 250)
        pdf.cell(0, 7, "  功能设计", fill=True)
        pdf.ln(10)

        ctrl = design_logic.get("controller")
        if isinstance(ctrl, dict):
            pdf.set_font("cjk", "", 10)
            pdf.cell(0, 6, f"主控芯片：{escape_pdf(str(ctrl.get('name', '—')))}")
            pdf.ln(8)

        func_lines = design_logic.get("function_lines")
        if isinstance(func_lines, list) and func_lines:
            pdf.set_font("cjk", "", 10)
            pdf.cell(0, 6, "核心功能：")
            pdf.ln(7)
            pdf.set_font("cjk", "", 9)
            for line in func_lines:
                pdf.cell(8, 5, "")
                pdf.cell(0, 5, f"• {escape_pdf(str(line))}")
                pdf.ln(5)
            pdf.ln(4)

        oled_pages = design_logic.get("oled_pages")
        if isinstance(oled_pages, list) and oled_pages:
            pdf.set_font("cjk", "", 10)
            pdf.cell(0, 6, f"OLED 页面设计（共 {len(oled_pages)} 页）：")
            pdf.ln(8)
            pdf.set_font("cjk", "", 9)
            for i, page in enumerate(oled_pages):
                if not isinstance(page, dict):
                    continue
                title = page.get("title", f"页面 {i + 1}")
                pdf.set_font("cjk", "", 9)
                pdf.cell(0, 5, f"页面 {i + 1}：{escape_pdf(str(title))}")
                pdf.ln(6)
                ...
                if isinstance(lines, list):
                    for line in lines:
                        ...
                        pdf.cell(8, 5, "")
                        pdf.cell(0, 5, escape_pdf(str(line)))
                        pdf.ln(5)
                pdf.set_text_color(0, 0, 0)
                pdf.ln(3)

        display_design = design_logic.get("display_design")
        if isinstance(display_design, list) and display_design:
            pdf.set_font("cjk", "", 10)
            pdf.cell(0, 6, "显示设计说明：")
            pdf.ln(7)
            pdf.set_font("cjk", "", 9)
            for item in display_design:
                pdf.cell(8, 5, "")
                pdf.cell(0, 5, f"• {escape_pdf(str(item))}")
                pdf.ln(5)
            pdf.ln(3)

        networking = design_logic.get("networking")
        if isinstance(networking, dict) and networking.get("enabled"):
            pdf.set_font("cjk", "", 10)
            pdf.cell(0, 6, f"联网功能：{escape_pdf(str(networking.get('note', '')))}")
            pdf.ln(6)
    ...
    pdf.cell(0, 10, f"白砾电子 bailab.space  ·  第 {pdf.page_no()} 页", align="C")
        ...
        try:
            ...
            if path == "/api/solution/pdf":
                ...
                if not isinstance(components, dict):
                    components = {}
                design_logic = data.get("design_logic", {})
                if not isinstance(design_logic, dict):
                    design_logic = {}
                pdf_buffer = generate_solution_pdf(topic, scores, components, design_logic)
                if pdf_buffer is None:
                    self.send_json(HTTPStatus.SERVICE_UNAVAILABLE, {"error": "PDF 生成功能暂不可用，请确认已安装 fpdf2 及中文字体"})
                    return
                body = pdf_buffer.getvalue()
                safe = topic.replace("/", "-").replace("\\", "-")[:60]
                filename = f"{safe}_功能方案.pdf" if safe else "功能方案.pdf"
                self.send_response(HTTPStatus.OK)
                ...
                self.wfile.write(body)
                # 同时记录 PDF 导出事件
                ACTIVITY.record_pdf_export(self.visitor_id(), topic, "opened")
                return
